...
_TOKENIZER_FOR_DOC = "PetlRobertaTokenizer"
...
# Copied from transformers.models.bert.modeling_bert.BertSelfAttention with Bert->PetlRoberta
class PetlRobertaSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in PetlRobertaModel forward() function)
...
class PetlRobertaSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
# Copied from transformers.models.bert.modeling_bert.BertAttention with Bert->PetlRoberta
class PetlRobertaAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = PetlRobertaSelfAttention(config, cache_key='self')
        self.output = PetlRobertaSelfOutput(config)
...
class PetlRobertaOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        if config.ffn_mode == 'lora':
...
# Copied from transformers.models.bert.modeling_bert.BertLayer with Bert->PetlRoberta
class PetlRobertaLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = PetlRobertaAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = PetlRobertaAttention(config)
        self.intermediate = PetlRobertaIntermediate(config)
        self.output = PetlRobertaOutput(config)
...
# Copied from transformers.models.bert.modeling_bert.BertEncoder with Bert->PetlRoberta
class PetlRobertaEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([PetlRobertaLayer(config) for _ in range(config.num_hidden_layers)])
...
class PetlRobertaPooler(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
...
class PetlRobertaPreTrainedModel(PreTrainedModel):
    """
...
ROBERTA_START_DOCSTRING = r"""
    ...
    Parameters:
        config (:class:`~transformers.PetlRobertaConfig`): Model configuration class with all the parameters of the
...
ROBERTA_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`({0})`):
            ...
            Indices can be obtained using :class:`~transformers.PetlRobertaTokenizer`. See
...
class PetlRobertaModel(PetlRobertaPreTrainedModel):
    """
    ...
    # Copied from transformers.models.bert.modeling_bert.BertModel.__init__ with Bert->PetlRoberta
    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
        self.config = config

        self.embeddings = PetlRobertaEmbeddings(config)
        self.encoder = PetlRobertaEncoder(config)

        self.pooler = PetlRobertaPooler(config) if add_pooling_layer else None
...
class PetlRobertaForCausalLM(PetlRobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `PetlRobertaLMHeadModel` as a standalone, add `is_decoder=True.`")

        self.roberta = PetlRobertaModel(config, add_pooling_layer=False)
        self.lm_head = PetlRobertaLMHead(config)
    ...
    def forward(
        ...
        Example::
            ...
            >>> from transformers import PetlRobertaTokenizer, PetlRobertaForCausalLM, PetlRobertaConfig
            >>> import torch

            >>> tokenizer = PetlRobertaTokenizer.from_pretrained('roberta-base')
            >>> config = PetlRobertaConfig.from_pretrained("roberta-base")
            >>> config.is_decoder = True
            >>> model = PetlRobertaForCausalLM.from_pretrained('roberta-base', config=config)
...
class PetlRobertaForMaskedLM(PetlRobertaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids", r"lm_head.decoder.bias"]
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `PetlRobertaForMaskedLM` make sure `config.is_decoder=False` for "
...
class PetlRobertaLMHead(nn.Module):
    """PetlRoberta Head for masked language modeling."""
...
class PetlRobertaForSequenceClassification(PetlRobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.roberta = PetlRobertaModel(config, add_pooling_layer=False)
        self.classifier = PetlRobertaClassificationHead(config)
...
@add_start_docstrings(
    """
    PetlRoberta Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
    softmax) e.g. for RocStories/SWAG tasks.
    """,
    ROBERTA_START_DOCSTRING,
)
class PetlRobertaForMultipleChoice(PetlRobertaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]

    def __init__(self, config):
        super().__init__(config)

        self.roberta = PetlRobertaModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
@add_start_docstrings(
    """
    PetlRoberta Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for
    Named-Entity-Recognition (NER) tasks.
    """,
    ROBERTA_START_DOCSTRING,
)
class PetlRobertaForTokenClassification(PetlRobertaPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
...
class PetlRobertaClassificationHead(nn.Module):
...
@add_start_docstrings(
    """
    PetlRoberta Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
    layers on top of the hidden-states output to compute `span start logits` and `span end logits`).
    """,
    ROBERTA_START_DOCSTRING,
)
class PetlRobertaForQuestionAnswering(PetlRobertaPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
